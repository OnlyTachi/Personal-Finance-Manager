...
export default function AddAssetPage() {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "valor_inicial" || name === "valor_taxa"
          ? parseFloat(value) || 0
          : value,
    }));
  };
  ...
  const inputClass =
    "w-full bg-gray-700 border border-gray-600 rounded-lg p-3 text-white placeholder-gray-400 focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-all";
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <form
        ...
        className="bg-surface p-8 rounded-xl shadow-lg border border-secondary/30 space-y-6"
      >
        <div>
          ...
          <input
            ...
            type="text"
            name="nome"
            placeholder="Ex: Bitcoin, Petrobras..."
            value={formData.nome}
        ...
        {isRV && (
          <div className="animate-in fade-in slide-in-from-top-2 duration-300">
            <label className={labelClass}>
              {formData.tipo_indexador === "CRYPTO"
                ? "ID CoinGecko"
                : "Ticker B3"}
            </label>
            <input
              ...
              name="ticker"
              placeholder={
                formData.tipo_indexador === "CRYPTO" ? "bitcoin" : "PETR4.SA"
              }
            ...
            {formData.tipo_indexador === "CRYPTO" && (
              <p className="text-xs text-yellow-500/80 mt-2">
                ⚠️ Use o ID da CoinGecko (ex: 'bitcoin').
              </p>
            )}
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-primary hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-lg transition-all flex items-center justify-center gap-2 mt-8 shadow-lg shadow-blue-900/20"
        >
          {loading ? (
            "Salvando..."
          ) : (
            <>
              <Save className="w-5 h-5" /> Salvar Investimento
            </>
          )}
        </button>
      </form>
