...
export default function AssetDetailsPage() {
  ...
  const handleDeleteAsset = async () => {
    if (
      confirm(
        "ATENÇÃO: Isso excluirá o ativo e TODO o histórico dele. Não há volta.",
      )
    ) {
      try {
  ...
  const handleSimulate = async () => {
    ...
    setIsProcessing(true);
    try {
      const result = await investmentsService.api.post(
        "/investments/transactions/simulate-withdrawal",
        {
          ativo_id: id,
  ...
  const handleUpdateTransaction = async (e) => {
    ...
    try {
      ...
      await investmentsService.updateTransaction(
  ...
  const handleDeleteTransaction = async (transacaoId) => {
    if (
      confirm("Tem certeza? Isso irá reverter o impacto no saldo do ativo.")
    ) {
      try {
  ...
  if (!asset)
    return (
      <div className="p-10 text-center text-red-400">Ativo não encontrado.</div>
    );

  const sortedTransactions = [...asset.transacoes].sort(
    (a, b) => new Date(b.timestamp) - new Date(a.timestamp),
  );
  const isRV =
    ["B3", "CRYPTO", "USA", "Ações", "FIIs", "Criptomoedas"].includes(
      asset.tipo_indexador,
    ) || ["Ações", "FIIs", "Criptomoedas"].includes(asset.categoria);
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          {editingAsset ? (
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setAssetForm({ ...assetForm, nome: e.target.value })
                }
                className="bg-slate-700 text-white text-2xl font-bold rounded p-1 border border-slate-500 outline-none"
              />
              <button
                type="submit"
          ...
          <p className="text-gray-400">
            {asset.tipo_indexador} • {asset.categoria}{" "}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-red-900/50 hover:bg-red-900 text-red-200 px-4 py-2 rounded flex items-center gap-2 border border-red-800 ml-4"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
          ...
          <p className="text-2xl font-bold text-white">
            R${" "}
            {asset.valor_atual_bruto.toLocaleString("pt-BR", {
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-red-900/30">
          ...
          <p className="text-2xl font-bold text-red-400">
            R${" "}
            {asset.imposto_estimado?.toLocaleString("pt-BR", {
              minimumFractionDigits: 2,
            }) || "0,00"}
          </p>
        </div>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
          ...
          <p className="text-2xl font-bold text-green-400">
            R${" "}
            {asset.valor_liquido_estimado.toLocaleString("pt-BR", {
              minimumFractionDigits: 2,
            })}
          </p>
        </div>
            ...
            {sortedTransactions.map((t) => (
              <tr key={t.id} className="hover:bg-slate-700/50 group">
                ...
                <td className="px-6 py-3 text-right text-red-400">
                  {t.iof_pago + t.ir_pago > 0
                    ? `- R$ ${(t.iof_pago + t.ir_pago).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
                    : "-"}
                </td>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            <h2 className="text-xl font-bold mb-4 text-white">
              {modalType === "Aporte" ? "💰 Novo Aporte" : "💸 Realizar Saque"}
            </h2>
              ...
              <div className="space-y-4">
                <div className="bg-slate-900 p-4 rounded border border-slate-700 space-y-2">
                  <div className="flex justify-between text-sm text-gray-400">
                    <span>Bruto</span>
                    <span>R$ {simulation.valor_bruto.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm text-red-400">
                    <span>Impostos</span>
                    <span>- R$ {simulation.total_imposto.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between font-bold text-green-400 border-t border-slate-700 pt-2">
                    <span>Líquido</span>
                    <span>R$ {simulation.valor_liquido.toFixed(2)}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            <h2 className="text-xl font-bold mb-4 text-white flex items-center gap-2">
              <Pencil className="w-5 h-5 text-blue-400" /> Editar Transação
            </h2>
