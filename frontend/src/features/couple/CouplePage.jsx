...
export default function CouplePage() {
    ...
    try {
      ...
      if (response.status === "linked") {
        ...
        const formattedHistory = histData.map((item) => ({
          ...item,
          dateStr: new Date(item.timestamp).toLocaleDateString("pt-BR", {
            day: "2-digit",
            month: "2-digit",
          }),
  ...
  const handleLink = async (e) => {
    ...
    try {
      ...
      alert(
        `Convite enviado! Peça para ${partnerUsername} te adicionar também.`,
      );
      fetchCoupleData();
    } catch (error) {
  ...
  const handleSaveGoal = async (e) => {
    ...
    try {
      const payload = {
        ...
        data_limite: goalForm.data_limite
          ? new Date(goalForm.data_limite).toISOString()
          : null,
        cor: goalForm.cor,
      };
  ...
  const handleSimulate = async (e) => {
    ...
    try {
      ...
      const aporteTotal =
        parseFloat(simForm.aporte_user) + parseFloat(simForm.aporte_partner);
  ...
  const allocationChartData = data?.combined?.allocation
    ? Object.entries(data.combined.allocation).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Visão compartilhada para objetivos em comum.
          </p>
        </div>
      ...
      {status === "no_partner" && (
        <div className="max-w-md mx-auto bg-surface p-8 rounded-2xl border border-slate-700 shadow-xl text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-4">
            Conecte-se com seu amor
          </h2>
          <p className="text-gray-400 mb-8">
            Adicione o <strong>username</strong> da sua parceira(o) para verem o
            patrimônio somado.
          </p>
          <form onSubmit={handleLink} className="flex gap-2">
            ...
            <button
              type="submit"
              className="bg-pink-600 hover:bg-pink-500 text-white font-bold py-3 px-6 rounded-lg transition-colors"
            >
              Conectar
            </button>
          </form>
      ...
      {status === "pending_approval" && (
        <div className="max-w-md mx-auto bg-surface p-8 rounded-2xl border border-yellow-500/30 shadow-xl text-center">
          ...
          <p className="text-gray-300 mb-6">
            Você adicionou <strong>{partnerName}</strong>. Aguardando
            confirmação.
          </p>
          <button
            onClick={handleUnlink}
        ...
        <div className="animate-in fade-in duration-500 space-y-8">
          ...
          <section>
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-white text-lg flex items-center gap-2">
                <Target className="w-6 h-6 text-emerald-400" /> Metas
                Compartilhadas
              </h3>
              <button
                onClick={() => {
            ...
            {goals.length === 0 ? (
              <div className="text-center p-8 bg-slate-900/50 rounded-2xl border border-slate-800 border-dashed">
                ...
                <p className="text-gray-500 text-sm">
                  Definam objetivos juntos! Nenhuma meta criada ainda.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {goals.map((g) => {
                  const percent = Math.min(
                    100,
                    (g.valor_atual / g.valor_alvo) * 100,
                  );
                  ...
                  return (
                    <div
                      key={g.id}
                      className="bg-surface rounded-xl border border-slate-700 p-5 relative group hover:border-slate-500 transition-colors"
                    >
                        ...
                        {isOwner && (
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="p-1.5 text-gray-400 hover:text-white bg-slate-800 rounded"
                            >
                              <Pencil className="w-3 h-3" />
                            </button>
                            <button
                              onClick={() => handleDeleteGoal(g.id)}
                              className="p-1.5 text-gray-400 hover:text-red-400 bg-slate-800 rounded"
                            >
                      ...
                      {g.data_limite && (
                        <div className="mt-3 pt-3 border-t border-slate-800 flex items-center gap-2 text-xs text-gray-500">
                          ...
                          {new Date(g.data_limite).toLocaleDateString("pt-BR")}
                        </div>
                      )}
                    </div>
                  );
            ...
            <section className="bg-gradient-to-r from-violet-900/40 to-fuchsia-900/40 rounded-2xl border border-fuchsia-500/30 p-6 relative overflow-hidden shadow-2xl">
              ...
              <div className="relative z-10">
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-6">
                  {/* Card Usuário */}
                  <div
                    className={`p-4 rounded-xl border transition-all ${battle.saver_winner === battle.user.username ? "bg-fuchsia-500/20 border-fuchsia-400 shadow-lg shadow-fuchsia-900/20" : "bg-slate-900/50 border-slate-700"}`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-white flex items-center gap-2">
                        {battle.saver_winner === battle.user.username && (
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <span className="text-3xl font-black text-white">
                        {battle.user.savings_rate.toFixed(0)}%
                      </span>
                      <span className="text-sm text-gray-400 mb-1">
                        ({renderValue(battle.user.saved)})
                      </span>
                    </div>
                    <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.max(0, Math.min(100, battle.user.savings_rate))}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  {/* Card Parceiro */}
                  <div
                    className={`p-4 rounded-xl border transition-all ${battle.saver_winner === battle.partner.username ? "bg-fuchsia-500/20 border-fuchsia-400 shadow-lg shadow-fuchsia-900/20" : "bg-slate-900/50 border-slate-700"}`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-white flex items-center gap-2">
                        {battle.saver_winner === battle.partner.username && (
                          <Crown className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                        )}
                    ...
                    <div className="flex items-end gap-2 mb-2">
                      <span className="text-3xl font-black text-white">
                        {battle.partner.savings_rate.toFixed(0)}%
                      </span>
                      <span className="text-sm text-gray-400 mb-1">
                        ({renderValue(battle.partner.saved)})
                      </span>
                    </div>
                    <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.max(0, Math.min(100, battle.partner.savings_rate))}%`,
                        }}
                      ></div>
                    </div>
                  </div>
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 border-b border-slate-700 pb-4 gap-2">
              <h3 className="font-bold text-white text-lg flex items-center gap-2">
                <Scale className="w-6 h-6 text-yellow-400" /> Acerto de Contas
                do Mês
              </h3>
              <span className="text-xs text-gray-400 bg-slate-950 px-2 py-1 rounded">
                Despesas "Compartilhado"
              </span>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-4">
              ...
              <div className="text-center flex-1 w-full md:w-auto bg-slate-900/30 p-3 rounded-lg md:bg-transparent">
                ...
                <p className="text-xl font-bold text-white">
                  {renderValue(settlement.total_paid_by_user || 0)}
                </p>
              </div>
              ...
              <div className="flex-1 w-full md:w-auto flex flex-col items-center">
                {settlementAmount < 1 ? (
                  ...
                  <div
                    className={`flex flex-col items-center p-4 rounded-xl border w-full ${userOwes ? "bg-red-900/10 border-red-500/30" : "bg-green-900/10 border-green-500/30"}`}
                  >
                    ...
                    <p
                      className={`text-3xl font-black mt-2 ${userOwes ? "text-red-500" : "text-green-500"}`}
                    >
              ...
              <div className="text-center flex-1 w-full md:w-auto bg-slate-900/30 p-3 rounded-lg md:bg-transparent">
                <p className="text-xs text-gray-400 uppercase font-bold">
                  {data.partner.username} Pagou
                </p>
                <p className="text-xl font-bold text-white">
                  {renderValue(settlement.total_paid_by_partner || 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-2xl border border-slate-700 shadow-lg relative overflow-hidden group">
              ...
              <p className="text-3xl font-bold text-white">
                {renderValue(data.combined.total_assets)}
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-2xl border border-slate-700 shadow-lg relative overflow-hidden group">
              ...
              <p className="text-3xl font-bold text-red-400">
                {renderValue(data.combined.total_liabilities)}
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-pink-900/40 to-purple-900/40 p-6 rounded-2xl border border-pink-500/30 shadow-lg relative overflow-hidden group">
              ...
              <h3 className="text-pink-200 text-sm uppercase tracking-wider mb-2">
                Patrimônio Líquido Casal
              </h3>
              <p className="text-4xl font-bold text-white">
                {renderValue(data.combined.net_worth)}
              </p>
            </div>
          </div>
          ...
          <div className="bg-surface p-6 rounded-2xl border border-slate-700 shadow-lg h-80">
            <h3 className="font-bold text-white mb-4 flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-purple-400" /> Evolução
              Patrimonial Combinada
            </h3>
            {history.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={history}
                  margin={{ top: 10, right: 30, left: -5, bottom: 14 }}
                >
                  <defs>
                    <linearGradient
                      id="colorCouple"
                  ...
                  <YAxis
                    ...
                    axisLine={false}
                    tickFormatter={(val) =>
                      privacyMode ? "•••" : `R$${(val / 1000).toFixed(0)}k`
                    }
                  />
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-surface p-6 rounded-2xl border border-slate-700 h-80 shadow-lg">
              <h3 className="font-bold text-white mb-4 flex items-center gap-2">
                <UserPlus className="w-4 h-4 text-blue-400" /> Contribuição no
                Patrimônio
              </h3>
                    ...
                    {data &&
                      [0, 1].map((entry, index) => (
                        <Cell
                          ...
                          fill={
                            COLORS_CONTRIBUTION[
                              index % COLORS_CONTRIBUTION.length
                            ]
                          }
                          stroke="rgba(0,0,0,0)"
                        />
                    ...
                    >
                      {allocationChartData.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            COLORS_ALLOCATION[index % COLORS_ALLOCATION.length]
                          }
                          stroke="rgba(0,0,0,0)"
                        />
                      ))}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-surface p-6 rounded-2xl border border-slate-700 w-full max-w-lg shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-2xl font-bold text-white mb-1 flex items-center gap-2">
              <Calculator className="text-pink-500 w-6 h-6" /> Projetar Futuro
              Juntos
            </h2>
            <p className="text-gray-400 text-sm mb-4">
              Simule o crescimento do patrimônio combinado.
            </p>
            ...
            {!simResult ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={simForm.aporte_user}
                      onChange={(e) =>
                        setSimForm({ ...simForm, aporte_user: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none"
                      value={simForm.aporte_partner}
                      onChange={(e) =>
                        setSimForm({
                          ...simForm,
                          aporte_partner: e.target.value,
                        })
                      }
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={simForm.anos}
                      onChange={(e) =>
                        setSimForm({ ...simForm, anos: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={simForm.taxa}
                      onChange={(e) =>
                        setSimForm({ ...simForm, taxa: e.target.value })
                      }
                    />
                  </div>
                </div>
                <div className="bg-slate-900/50 p-3 rounded text-center text-sm text-gray-400">
                  Aporte mensal conjunto:{" "}
                  <span className="text-white font-bold">
                    R${" "}
                    {(
                      parseFloat(simForm.aporte_user || 0) +
                      parseFloat(simForm.aporte_partner || 0)
              ...
              <div className="space-y-6 mt-4 animate-in slide-in-from-bottom-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-900 p-4 rounded-lg border border-slate-700">
                    ...
                    <p className="text-lg font-bold text-white">
                      R$ {simResult.total_investido.toLocaleString("pt-BR")}
                    </p>
                  </div>
                  <div className="bg-slate-900 p-4 rounded-lg border border-pink-500/30 bg-pink-900/10">
                    <p className="text-xs text-pink-300 uppercase">
                      Patrimônio Final
                    </p>
                    <p className="text-lg font-bold text-white">
                      R$ {simResult.valor_liquido.toLocaleString("pt-BR")}
                    </p>
                  </div>
                </div>
                <div className="h-48 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                      data={simResult.projecao_mensal.filter(
                        (_, i) => i % 12 === 0,
                      )}
                    >
                      <defs>
            ...
            <form onSubmit={handleSaveGoal} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  value={goalForm.nome}
                  onChange={(e) =>
                    setGoalForm({ ...goalForm, nome: e.target.value })
                  }
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setGoalForm({ ...goalForm, valor_alvo: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setGoalForm({ ...goalForm, valor_atual: e.target.value })
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setGoalForm({ ...goalForm, data_limite: e.target.value })
                    }
                  />
                </div>
                <div>
              ...
              <button
                type="submit"
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-lg transition-colors mt-2 shadow-lg shadow-emerald-900/20"
              >
                Salvar Meta
              </button>
            </form>
