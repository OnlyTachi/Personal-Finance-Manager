...
export default function PassivoDetailsPage() {
  ...
  const handleToggleParcela = async (parcelaId) => {
    try {
      const updatedPassivo = await investmentsService.toggleParcela(
        id,
        parcelaId,
      );
  ...
  const progresso =
    parcelasTotal > 0 ? (parcelasPagas / parcelasTotal) * 100 : 0;
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8 bg-surface p-6 rounded-xl border border-red-900/30">
        <div>
          ...
          <div className="flex gap-4 text-sm text-gray-400">
            ...
            <span className="flex items-center gap-1">
              <Calendar className="w-4 h-4" /> {passivo.prazo_meses} Meses
            </span>
          </div>
        </div>
        <div className="text-right">
          ...
          <p className="text-4xl font-bold text-red-400">
            R${" "}
            {passivo.saldo_devedor.toLocaleString("pt-BR", {
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-white flex items-center gap-2">
          <CheckCircle2 className="text-green-500" /> Parcelas
        </h2>
        <button
          onClick={handleDelete}
          className="text-red-500 hover:text-red-400 text-sm flex items-center gap-1 border border-red-900/50 px-3 py-1 rounded hover:bg-red-900/20 transition-colors"
        >
          ...
          <table className="w-full text-left text-sm text-gray-300">
            ...
            <tbody className="divide-y divide-secondary/30">
              {parcelas.map((p) => {
                ...
                const isLate =
                  !isPaid && new Date(p.data_vencimento) < new Date();

                return (
                  <tr
                    key={p.id}
                    className={`hover:bg-slate-800/50 transition-colors ${isPaid ? "opacity-50 hover:opacity-100" : ""}`}
                  >
                    ...
                    <td className="px-6 py-3">
                      ...
                      {isLate && (
                        <span className="ml-2 text-xs bg-red-900/50 text-red-200 px-2 py-0.5 rounded">
                          Atrasada
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-3 text-center">
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold ${isPaid ? "bg-green-900/30 text-green-400" : "bg-yellow-900/30 text-yellow-400"}`}
                      >
                        {p.status}
                      </span>
                    </td>
