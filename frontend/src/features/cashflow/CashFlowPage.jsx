...
import CsvMapperModal from "../../components/ui/CsvMapperModal";
...
export default function CashFlowPage() {
  ...
  const getOrigemIcon = (origem) => {
    switch (origem) {
      case "TELEGRAM":
        return <Bot className="w-4 h-4 text-blue-400" title="Telegram Bot" />;
      case "IMPORT":
        return (
          <FileText
      ...
      case "OCR":
        return (
          <Smartphone className="w-4 h-4 text-purple-400" title="Scanner OCR" />
        );
      default:
        return null;
    }
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto pb-24 bg-background min-h-screen font-sans">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6">
        <div>
          <button
            ...
            className="flex items-center text-gray-400 hover:text-white mb-2 transition-colors text-sm"
          >
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          <button
            ...
            className="flex-1 lg:flex-none bg-slate-800 hover:bg-slate-700 text-white px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all border border-slate-700"
          >
          ...
          <button
            ...
            className="flex-1 lg:flex-none bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 font-bold transition-all shadow-lg shadow-blue-900/20 hover:-translate-y-0.5"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-slate-800/50 p-5 rounded-xl border border-slate-700 shadow-sm relative overflow-hidden">
          <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1 flex items-center gap-1">
        ...
        <div className="bg-slate-800/50 p-5 rounded-xl border border-slate-700 shadow-sm relative overflow-hidden">
          ...
          <p className="text-2xl font-bold text-white">
            R${" "}
            {summary.saidas.toLocaleString("pt-BR", {
              minimumFractionDigits: 2,
            })}
          </p>
        </div>
        <div
          className={`bg-slate-800/50 p-5 rounded-xl border border-slate-700 shadow-sm`}
        >
          <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">
            Saldo do Mês
          </p>
          <p
            className={`text-2xl font-bold ${summary.saldo >= 0 ? "text-blue-400" : "text-red-400"}`}
          >
            R${" "}
        ...
        <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 flex flex-col justify-center relative overflow-hidden">
          <div className="flex justify-between items-center mb-1">
            ...
            <span
              className={`text-xl font-black ${savingsRate > 20 ? "text-emerald-400" : "text-yellow-400"}`}
            >
          ...
          <div className="w-full h-1.5 bg-slate-700 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-1000 ${savingsRate > 20 ? "bg-emerald-500" : "bg-yellow-500"}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 animate-in slide-in-from-bottom-4">
          <div className="lg:col-span-2 bg-surface p-6 rounded-xl border border-slate-700/50 shadow-lg">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-6 flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-blue-500" /> Fluxo Diário
            </h3>
            <div className="h-56">
          ...
          <div className="bg-surface p-6 rounded-xl border border-slate-700/50 shadow-lg">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-2">
              <PieChart className="w-4 h-4 text-purple-500" /> Categorias
            </h3>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <RePieChart>
              ...
              {filteredTransactions.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          {t.conciliado && (
                            <span className="text-[10px] bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-500/20 font-bold tracking-wide">
                              OK
                            </span>
                          )}
                          {t.shared && (
                            <span className="text-[10px] bg-pink-500/10 text-pink-400 px-1.5 py-0.5 rounded border border-pink-500/20 font-bold tracking-wide flex items-center gap-1">
                              <Heart className="w-2 h-2 fill-current" /> Casal
                            </span>
                          )}
                        </div>
                        ...
                        {historicoReal && historicoReal !== t.descricao && (
                          <span
                            className="text-xs text-gray-500 truncate max-w-[350px] font-medium"
                            title={historicoReal}
                          >
                            {historicoReal}
                          </span>
                    ...
                    <td
                      className={`px-6 py-4 text-right font-bold text-sm ${t.valor >= 0 ? "text-emerald-400" : "text-red-400"}`}
                    >
                      R${" "}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="flex gap-2 p-1 bg-slate-900 rounded-lg border border-slate-700">
                <button
                  ...
                  className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${form.tipo === "SAIDA" ? "bg-red-600 text-white shadow" : "text-gray-400 hover:text-white"}`}
                >
                  DESPESA
                </button>
                <button
                  ...
                  className={`flex-1 py-2 rounded-md text-sm font-bold transition-all ${form.tipo === "ENTRADA" ? "bg-emerald-600 text-white shadow" : "text-gray-400 hover:text-white"}`}
                >
                  RECEITA
                </button>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none focus:border-blue-500 transition-colors"
                />
              </div>

              <div>
                <label className="text-xs text-gray-400 uppercase font-bold mb-1 flex items-center gap-1">
                  <FileClock className="w-3 h-3" /> Detalhes / Histórico
                </label>
                <input
                  placeholder="Ex: COMPRA ELO *SUPERMERCADO"
                  value={form.historico}
                  onChange={(e) =>
                    setForm({ ...form, historico: e.target.value })
                  }
                  className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white outline-none focus:border-blue-500 transition-colors text-sm font-mono"
                />
              </div>
              ...
              {form.tipo === "SAIDA" && (
                <div className="flex items-center gap-3 bg-pink-900/20 p-3 rounded-lg border border-pink-500/30">
                  <input
                    ...
                    className="w-5 h-5 text-pink-600 bg-slate-900 border-slate-600 rounded focus:ring-pink-500"
                  />
                  <label
                    htmlFor="shared"
                    className="text-sm text-pink-200 font-medium cursor-pointer flex items-center gap-2"
                  >
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold transition-colors shadow-lg shadow-blue-900/20"
                >
                  Salvar
                </button>
              </div>
