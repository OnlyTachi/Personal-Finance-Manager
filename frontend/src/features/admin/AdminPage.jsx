...
export default function AdminPage() {
  ...
  const handleUpdatePass = async (e) => {
    e.preventDefault();
    try {
      await investmentsService.adminUpdateUser(passwordForm.username, {
  ...
  const toggleAdmin = async (u) => {
    if (
      !confirm(
        `Tornar ${u.username} ${u.is_admin ? "Usuário Comum" : "Admin"}?`,
      )
    )
      return;
    try {
      await investmentsService.adminUpdateUser(u.username, {
        is_admin: !u.is_admin,
      });
      fetchUsers();
    } catch (err) {
  ...
  const handleDelete = async (username) => {
    if (
      !confirm(
        `Tem certeza que deseja apagar ${username}? Todos os dados financeiros dele serão perdidos.`,
      )
    )
      return;
    try {
            ...
            {users.map((u) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  {u.is_admin ? (
                    <span className="bg-purple-900/30 text-purple-300 px-2 py-1 rounded text-xs border border-purple-700/50 font-bold">
                      ADMIN
                    </span>
                  ) : (
                    <span className="bg-slate-700/50 text-slate-300 px-2 py-1 rounded text-xs border border-slate-600/50">
                      USER
                    </span>
                  )}
                </td>
                ...
                <td className="px-6 py-4 flex justify-end gap-2">
                  <button
                    onClick={() => handleAnalyze(u.username)}
                    title="Analisar Patrimônio"
                  ...
                  <button
                    ...
                    className={`p-2 rounded transition-colors ${u.is_admin ? "hover:bg-gray-700 text-gray-400" : "hover:bg-purple-900/30 text-purple-400"}`}
                  >
                    {u.is_admin ? (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-surface p-6 rounded-xl border border-slate-700 w-full max-w-md relative shadow-2xl">
            ...
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <Activity className="text-blue-400" /> {statsUser.username}
            </h2>
            <div className="space-y-4">
              <div className="bg-slate-900 p-4 rounded-lg border border-slate-700 text-center">
                ...
                <p className="text-3xl font-bold text-green-400">
                  R${" "}
                  {statsUser.patrimonio_liquido.toLocaleString("pt-BR", {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-900 p-3 rounded-lg border border-slate-700">
                  ...
                  <p className="text-white font-bold">
                    R${" "}
                    {statsUser.total_ativos.toLocaleString("pt-BR", {
                      minimumFractionDigits: 2,
                    })}
                  </p>
                </div>
                <div className="bg-slate-900 p-3 rounded-lg border border-slate-700">
                  ...
                  <p className="text-red-400 font-bold">
                    R${" "}
                    {statsUser.total_dividas.toLocaleString("pt-BR", {
                      minimumFractionDigits: 2,
                    })}
                  </p>
                </div>
              ...
              <p className="text-xs text-center text-gray-600 mt-4">
                Membro desde{" "}
                {statsUser.data_cadastro
                  ? new Date(statsUser.data_cadastro).toLocaleDateString(
                      "pt-BR",
                    )
                  : "N/A"}
              </p>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-surface p-6 rounded-xl border border-slate-700 w-full max-w-sm shadow-2xl">
            <h3 className="font-bold text-white mb-4 text-lg">
              Nova senha para{" "}
              <span className="text-yellow-400">{passwordForm.username}</span>
            </h3>
            <form onSubmit={handleUpdatePass}>
              <input
                ...
                className="w-full bg-slate-900 border border-slate-600 rounded-lg p-3 text-white mb-6 focus:border-yellow-500 outline-none transition-colors"
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-white py-2 rounded-lg font-medium transition-colors"
                >
                  Salvar
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-surface p-6 rounded-xl border border-slate-700 w-full max-w-sm shadow-2xl">
            <h3 className="font-bold text-white mb-6 text-lg flex items-center gap-2">
              <Plus className="w-5 h-5 text-purple-400" /> Criar Usuário
            </h3>
