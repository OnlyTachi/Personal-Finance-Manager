      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400">
            Acompanhe seu crescimento e movimentações diárias.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm text-gray-300">
            <thead className="bg-slate-900/50 text-gray-400 sticky top-0 uppercase text-xs">
              <tr>
                ...
                <th className="px-6 py-4 text-green-400 text-right">
                  Aportes Dia
                </th>
                <th className="px-6 py-4 text-red-400 text-right">
                  Saques Dia
                </th>
              ...
              {[...history].reverse().map((item) => (
                <tr key={item.id} className="hover:bg-slate-800/50">
                  ...
                  <td className="px-6 py-3 text-right">
                    {item.total_aportes > 0 ? (
                      <span className="text-green-400 flex items-center justify-end gap-1">
                        ...
                        {item.total_aportes.toLocaleString("pt-BR", {
                  ...
                  <td className="px-6 py-3 text-right">
                    {item.total_saques > 0 ? (
                      <span className="text-red-400 flex items-center justify-end gap-1">
                        ...
                        {item.total_saques.toLocaleString("pt-BR", {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    ) : (
                  ...
                  <td className="px-6 py-3 text-right text-gray-500">
                    R${" "}
                    {item.valor_total_investido?.toLocaleString("pt-BR", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                  <td className="px-6 py-3 text-right font-bold text-white">
                    R${" "}
                    {item.valor_total_bruto.toLocaleString("pt-BR", {
                      minimumFractionDigits: 2,
                    })}
                  </td>
                </tr>
