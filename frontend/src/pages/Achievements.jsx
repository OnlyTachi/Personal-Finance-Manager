...
import { 
  ArrowLeft, Trophy, Lock, Footprints, Crown, Diamond, 
  ShieldCheck, PieChart, Heart, Bitcoin, Star, RefreshCw, AlertTriangle
} from 'lucide-react';

// Mapa de ícones (string do backend -> componente React)
const ICON_MAP = {
  Footprints, Trophy, Crown, Diamond, ShieldCheck, 
  PieChart, Heart, Bitcoin, Star
};
...
export default function AchievementsPage() {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-400 mb-6 max-w-md">
          Não foi possível carregar suas conquistas. O servidor pode estar reiniciando ou a tabela de gamificação ainda não foi criada.
        </p>
        <div className="flex gap-4">
            <button onClick={() => navigate('/dashboard')} className="px-6 py-2 rounded-lg border border-slate-700 text-gray-300 hover:bg-slate-800 transition-colors">
                Voltar
            </button>
            <button onClick={loadData} className="px-6 py-2 rounded-lg bg-primary text-white hover:bg-blue-600 transition-colors">
                Tentar Novamente
            </button>
        </div>
      </div>
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto pb-20 animate-in fade-in duration-500">
      <button onClick={() => navigate('/dashboard')} className="flex items-center text-gray-400 hover:text-white mb-6 transition-colors group">
      ...
      <div className="bg-gradient-to-r from-yellow-900/20 to-amber-900/20 border border-yellow-500/20 p-8 rounded-2xl mb-10 text-center relative overflow-hidden shadow-2xl">
        <div className="absolute top-0 right-0 p-8 opacity-5"><Trophy className="w-64 h-64 text-yellow-500"/></div>
        
        <h2 className="text-yellow-500/80 text-xs uppercase tracking-[0.3em] font-bold mb-3">Nível de Investidor</h2>
        ...
        <div className="inline-flex items-center gap-3 bg-slate-950/50 px-5 py-2.5 rounded-full border border-yellow-500/10 backdrop-blur-sm shadow-inner">
          ...
          <span className="text-yellow-100 font-bold text-lg">{status.earned_count}</span>
          <span className="text-gray-500 text-sm font-medium">de</span>
          <span className="text-gray-400 font-bold text-lg">{status.total_badges}</span>
          <span className="text-gray-500 text-sm font-medium uppercase tracking-wide ml-1">Desbloqueadas</span>
        </div>
        ...
        <div className="mt-8 max-w-lg mx-auto relative">
            <div className="h-2 bg-slate-800 rounded-full overflow-hidden border border-slate-700/50">
                <div 
                    ...
                    style={{width: `${(status.earned_count / status.total_badges) * 100}%`}}
                ></div>
            </div>
        ...
        {status.badges?.map((badge) => {
          ...
          return (
            ...
            >
              ...
              {/* Ícone */}
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-5 text-3xl shadow-lg transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3
                ${badge.earned ? `bg-slate-800 border border-slate-600 ${badge.color}` : 'bg-slate-950 border border-slate-800 text-gray-700'}`}
              >
                {badge.earned ? <IconComponent className="w-8 h-8"/> : <Lock className="w-6 h-6"/>}
              </div>

              {/* Textos */}
              <h4 className={`font-bold text-lg mb-2 leading-tight ${badge.earned ? 'text-white' : 'text-gray-500'}`}>
                {badge.name}
              </h4>
              ...
              {badge.earned && badge.earned_at && (
                <div className="mt-5 pt-4 border-t border-slate-700/50 flex justify-between items-center">
                  <span className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Conquistado</span>
