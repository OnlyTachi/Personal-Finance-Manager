...
import { analyzeFile, mapFile, importBulk, uploadLegacyPreview } from '../services/api';
import { X, Upload, ArrowRight, Check, AlertCircle, FileWarning, Sparkles, HelpCircle } from 'lucide-react';
...
export default function CsvMapperModal({ isOpen, onClose, onSuccess }) {
  ...
  const reset = () => {
    ...
    setPreviewData([]);
    setError(null);
    setMapping({ date_col: '', history_col: '', description_col: '', amount_col: '', use_history_for_ai: false });
  };
    ...
    try {
      if (isLegacyFormat) {
        ...
        if (res.headers && res.headers.length > 0) {
          ...
          // Auto-mapeamento
          const autoMap = { date_col: '', history_col: '', description_col: '', amount_col: '', use_history_for_ai: false };
          res.headers.forEach(h => {
            const lower = String(h).toLowerCase();
            if (lower.includes('data') || lower.includes('date')) autoMap.date_col = h;
            if (lower.includes('valor') || lower.includes('amount')) autoMap.amount_col = h;

            if (lower.includes('hist') || lower.includes('memo') || lower.includes('complemento')) {
              autoMap.history_col = h;
            }
            if (lower.includes('desc') || lower.includes('estab') || lower.includes('loja')) {
              autoMap.description_col = h;
            }
          });

          if (autoMap.history_col && (!autoMap.description_col || autoMap.description_col.toLowerCase().includes('tipo'))) {
  ...
  const handleMappingSubmit = async () => {
    ...
    try {
      ...
      if (file && file.name.endsWith('.csv') && mapping.history_col) {
        try {
          const localHistoryList = await parseLocalCSVForHistory(file, mapping.history_col);
          if (localHistoryList.length > 0) {
            processedData = processedData.map((item, index) => ({
              ...item,
              historico: item.historico || item.memo || localHistoryList[index] || ''
            }));
          }
        } catch (parseErr) {
      ...
      <div className="bg-surface rounded-xl border border-slate-700 shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] relative">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-700 bg-slate-800 rounded-t-xl shrink-0">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            Importar Transações {step > 1 && step < 3 && <span className="text-sm font-normal text-gray-400 ml-2">Passo {step}/3</span>}
          </h2>
          <button onClick={handleClose} className="text-gray-400 hover:text-white transition-colors p-1 hover:bg-slate-700 rounded z-10">
            <X size={24} />
          </button>
        </div>
            ...
            <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-slate-600 rounded-xl bg-slate-900/50 hover:bg-slate-900 transition-all cursor-pointer relative group">
              ...
              {loading ? (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-blue-400 font-medium animate-pulse">Analisando arquivo...</span>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex gap-2">
                    <span className="bg-slate-800 text-gray-400 px-2 py-1 rounded text-xs border border-slate-700">Nubank</span>
                    <span className="bg-slate-800 text-gray-400 px-2 py-1 rounded text-xs border border-slate-700">Inter</span>
                    <span className="bg-slate-800 text-gray-400 px-2 py-1 rounded text-xs border border-slate-700">XP</span>
                  </div>
                </>
              )}
            </div>
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
              <div className="bg-blue-900/20 p-4 rounded-xl text-sm text-blue-200 flex items-start gap-3 border border-blue-500/20">
                <FileWarning size={20} className="mt-0.5 shrink-0 text-blue-400" />
                <div>
                  <p className="font-bold mb-1 text-white">Identifique as Colunas</p>
                  <p className="text-blue-200/80">O sistema tentou adivinhar, mas confira se as colunas do seu arquivo correspondem aos campos abaixo.</p>
                </div>
              </div>
              ...
              <div className="p-5 bg-purple-900/20 border border-purple-500/20 rounded-xl flex flex-col md:flex-row items-center justify-between shadow-sm gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <span className="text-base font-bold text-white block">Inteligência Artificial</span>
                    <span className="text-sm text-gray-400">Qual coluna a IA deve ler para categorizar?</span>
                  </div>
                </div>

                <div className="flex items-center gap-3 bg-slate-900/50 p-2 rounded-lg border border-slate-700">
                  <span className={`text-xs font-bold uppercase transition-colors ${!mapping.use_history_for_ai ? 'text-purple-400' : 'text-gray-500'}`}>Descrição</span>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      className="sr-only peer"
                      checked={mapping.use_history_for_ai}
                      onChange={(e) => setMapping({ ...mapping, use_history_for_ai: e.target.checked })}
                    />
                    <div className="w-11 h-6 bg-slate-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600 transition-colors"></div>
                  </label>
                  <span className={`text-xs font-bold uppercase transition-colors ${mapping.use_history_for_ai ? 'text-purple-400' : 'text-gray-500'}`}>Histórico</span>
                </div>
              </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-xs text-left text-gray-300">
                      <thead>
                        <tr>
                          {headers.map((h, i) => (
                            <th key={i} className="px-4 py-3 font-bold border-b border-slate-700 whitespace-nowrap bg-slate-800/30 text-white">
                              {h}
                            </th>
                          ))}
                        </tr>
                        ...
                        {sampleRows.slice(0, 3).map((row, idx) => (
                          <tr key={idx}>
                            {row.map((cell, cellIdx) => (
                              <td key={cellIdx} className="px-4 py-3 border-b border-slate-800 whitespace-nowrap max-w-[200px] truncate text-gray-400">
                                {cell}
                              </td>
                            ))}
                          </tr>
              ...
              <div className="flex items-center justify-between mb-4 shrink-0">
                <div className="flex items-center gap-3">
                  <span className="bg-green-500/20 text-green-400 px-3 py-1 rounded-lg text-sm font-bold border border-green-500/30 flex items-center gap-2">
                    <Check className="w-4 h-4" /> {previewData.length} Lançamentos
                  </span>
                  <p className="text-sm text-gray-400">Confira se a categorização da IA está correta.</p>
                </div>
              </div>
                ...
                <div className="overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-slate-700">
                  <table className="min-w-full divide-y divide-slate-800">
                    <thead className="bg-slate-800 sticky top-0 shadow-lg z-10">
                      <tr>
                        <th className="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Data</th>
                        <th className="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Histórico / Descrição</th>
                        <th className="px-5 py-4 text-right text-xs font-bold text-gray-400 uppercase tracking-wider">Valor</th>
                        <th className="px-5 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Categoria (IA)</th>
                      </tr>
                    </thead>
                      ...
                      {previewData.map((t, idx) => (
                        <tr key={idx} className="hover:bg-slate-800/50 transition-colors group">
                          ...
                          <td className="px-5 py-3">
                            ...
                            {t.historico && (
                              <div className="text-xs text-gray-500 font-mono mt-0.5 max-w-sm truncate">{t.historico}</div>
                            )}
                          </td>

                          <td className={`px-5 py-3 text-sm text-right font-bold whitespace-nowrap ${t.valor < 0 ? 'text-red-400' : 'text-emerald-400'}`}>
        ...
        <div className="p-6 border-t border-slate-700 flex justify-end gap-3 bg-slate-800 rounded-b-xl shrink-0">
          {step > 1 && (
            <button
              onClick={() => setStep(step === 3 && !headers.length ? 1 : step - 1)}
          ...
          {step === 2 && (
            ...
            >
              {loading ? 'Processando...' : <>Pré-visualizar <ArrowRight size={16} /></>}
            </button>
          )}
          ...
          {step === 3 && (
            ...
            >
              {loading ? 'Salvando...' : <>Confirmar Importação <Check size={16} /></>}
            </button>
          )}
        </div>
      </div>
...
function MappingField({ label, value, onChange, options, description, required, aiActive }) {
  return (
    <div className={`rounded-xl transition-all duration-300 ${aiActive ? 'bg-purple-900/20 border border-purple-500/40 p-4 -m-4 shadow-lg scale-[1.02] z-10' : ''}`}>
